...
export const StyledButton = styled(StyledButtonBase)`
    ...
    border-radius: 8px;
    padding: 16px;
    border-color: ${({ theme, color, disabled }) => (disabled ? theme.colors.brand70 : color || theme.colors.white)};
    border-width: 1px;
`;

export const StyledLabel = styled.Text<StyledLabelInterface>`
    color: ${({ theme, color, disabled }) => (disabled ? theme.colors.brand70 : color || theme.colors.white)};
    font-style: normal;
